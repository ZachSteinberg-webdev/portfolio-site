...
const SpamSubmission = require('./models/spam-submission-model');
const { projects } = require('./data/projects.js');
const { skills } = require('./data/skills.js');
const { CustomError, tryCatch, errorHandler } = require('./utilities/errorHandling');
...
app.post(
	...
	tryCatch(async (req, res, next) => {
		if (req.body.spamTrap !== '') {
			const spamSubmissionDetails = {
				spamTrapData: req.body.spamTrap,
				name: req.body.name,
				email: req.body.email,
				message: req.body.message
			};
			console.log(spamSubmissionDetails);
			const newSpamSubmission = await new SpamSubmission(spamSubmissionDetails);
			await newSpamSubmission.save();
			res.status(200).end();
		} else {
