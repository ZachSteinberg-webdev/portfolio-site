...
const removeNextNextSibClass = (element, className)=>{
	...
	// Event handlers
const projectExpandButtonClickHandler = (event)=>{ // When a project container expand button is clicked...
	...
	if(previousParent!==null) {                      // If a previous parent doesn't exist, ignore it, else...
		previousParentOpacity = window.getComputedStyle(previousParent).opacity; // calculate the previous parent's opacity.
	};
	if(nextParent!==null) {                          // If a next parent doesn't exist, ignore it, else...
		nextParentOpacity = window.getComputedStyle(nextParent).opacity; // calculate the next parent's opacity.
	};
	if((previousParentOpacity==="1")||(nextParentOpacity==="1")) { // If no container is expanded, then...
		if(memojiContainerExpandButton.contains(target)) {           // if memoji project container was clicked, then...
			if(memojiContainerAlreadyClicked===false) {                // if memoji project container not yet been clicked...
				memojiContainerAlreadyClicked=true;                      // set memojiContainerAlreadyClicked to true...
				let message = document.createElement("p");               // and create a paragraph element...
				message.classList.add('message-insertion-point');        // and give it the class "message-insertion-point"...
				message.innerText="Nothing to see here!";                // and give it inner text "nothing to see here!"...
				zachMemojiWorkingContainer.prepend(message);             // and prepend it to the memoji container...
				setTimeout(()=>{                                         // and set a timeout to...
					message.classList.add('message-insertion-point-after-delay'); // add "message-insertion-point-after-delay"...
				},250);                                                         // after 250 milliseconds...
				// workingMemojiTextInsertionPoint.innerText="Nothing to see here!"; // or else, if memoji container has already been clicked, then...
			} else {
				memojiContainerAlreadyClicked=false;                     // set memojiContainerAlreadyClicked to false;
				let message = document.querySelector('.message-insertion-point'); // and select the message element...
				message.classList.remove('message-insertion-point-after-delay');  // and remove the class "message-insertion-point-after-delay"...
				setTimeout(()=>{                               // and set a timeout to...
					message.remove();                            // remove the message element from the memoji container...
				},250);                                        // after 250ms...
			};
		} else {                                           // or else, if memoji container was not clicked, then...
			projectExpandButtons.forEach((item)=>{           // for each button...
				if(item.contains(target)) {                    // if the button is the clicked button, then...
				...
				} else {                                                   // if the button is not the clicked button, then...
	...
	} else {                                                 // if one of the containers is expanded, then...
		if(memojiContainerExpandButton.contains(target)) {
			workingMemojiTextInsertionPoint.innerText="";
		} else {
			projectExpandButtons.forEach((item)=>{               // for each button...
				if(item.contains(target)) {                        // if the button is the clicked button, then...
